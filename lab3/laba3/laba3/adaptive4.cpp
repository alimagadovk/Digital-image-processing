...
unsigned int			cum_freq[LEVELS_OF_BRIGHTNESS * LEVELS_OF_BRIGHTNESS][NO_OF_SYMBOLS + 1]; //интервалы частот символов
...
{ // изначально все символы в сообщении считаем равновероятными 
	int i, j;
	for (i = 0; i < LEVELS_OF_BRIGHTNESS * LEVELS_OF_BRIGHTNESS; i++)
...
void encode(void)
{
	...
	while ((symbol = getc(in)) != EOF)
	{
		...
		mas_sym.push_back(symbol);
		m = std::min(symbol, vert_symbol);
		M = std::max(symbol, vert_symbol);
		temp_cum_freq = cum_freq[LEVELS_OF_BRIGHTNESS * M + m];
		i++;
		//printf("\ni = %d\n", i);
	}
...
//double entropy(bool with_eof = 0)
//{
//	double* p;
//	double H = 0;
//	if (with_eof)
...
//		{
//			p[i] = ((double)cum_freq[i + 1] - (double)cum_freq[i]) / cum_freq[NO_OF_SYMBOLS];
//			H += -p[i] * std::log2(p[i]);
//		}
//	}
//	else
...
//		for (int i = 0; i < NO_OF_CHARS - 1; i++)
//		{
//			p[i] = ((double)cum_freq[i + 2] - (double)cum_freq[i + 1]) / cum_freq[NO_OF_SYMBOLS];
//			H += -p[i] * std::log2(p[i]);
//		}
//	}
//	return H;
