...
PSNR2(my_ind) = 10*log10(512*512/sum(sum((I-I2).^2)))
s = dir('jp.ar');
the_size = s.bytes;
bpp2(my_ind) = 8 * the_size / 512^2
end
%%
% close all
% figure
% hold on
% grid on
plot(bpp2, PSNR2, '*r', 'LineWidth', 1.5)
p2 = plot(bpp2, PSNR2, 'r', 'LineWidth', 1.5, 'DisplayName', 'Обратный порядок');
xlabel('bpp')
ylabel('PSNR')
title('PSNR(bpp)')
legend([p1, p2], 'Location', 'northwest')
%%
% fileID = fopen('bpp2.txt','w');
% fprintf(fileID,'%.16f\n',bpp2);
% fclose(fileID);
% fileID = fopen('PSNR2.txt','w');
% fprintf(fileID,'%.16f\n',PSNR2);
% fclose(fileID);
%%
