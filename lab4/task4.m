...
PSNR4(my_ind) = 10*log10(512*512/sum(sum((I-I2).^2)))
s = dir('jp.ar');
the_size = s.bytes;
bpp4(my_ind) = 8 * the_size / 512^2
end
%%
close all
figure
...
p1 = plot(bpp1, PSNR1, 'b', 'LineWidth', 1.25, 'DisplayName', 'PSNR(bpp), task1');

plot(bpp4, PSNR4, '*r', 'LineWidth', 1.25)
p4 = plot(bpp4, PSNR4, 'r', 'LineWidth', 1.25, 'DisplayName', 'PSNR(bpp), task4');

xlabel('bpp')
ylabel('PSNR')
title('PSNR(bpp)')
legend([p1,p4], 'Location', 'northwest')
%%
% fileID = fopen('bpp4.txt','w');
% fprintf(fileID,'%.16f\n',bpp4);
% fclose(fileID);
% fileID = fopen('PSNR4.txt','w');
% fprintf(fileID,'%.16f\n',PSNR4);
% fclose(fileID);
%%
...
fclose(fileID);
fileID = fopen('bpp4.txt','r');
bpp4 = fscanf(fileID,'%f\n');
fclose(fileID);
fileID = fopen('PSNR4.txt','r');
PSNR4 = fscanf(fileID,'%f\n');
fclose(fileID);