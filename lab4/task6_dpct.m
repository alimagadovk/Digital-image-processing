...
imwrite(im2uint8(I2),strcat('6dpct_Q_quality=', num2str(quality(my_ind)), '_bpp=', num2str(bpp6(my_ind)), '.gif'),'gif');
end
%%
close all
figure
...
p1 = plot(bpp6, PSNR6, 'r', 'LineWidth', 1.5, 'DisplayName', 'PSNR(bpp), dpct');
xlabel('bpp')
ylabel('PSNR')
title('PSNR(bpp)')
...
p2 = plot(bpp6, SSIM6, 'r', 'LineWidth', 1.5, 'DisplayName', 'SSIM(bpp), dpct');
xlabel('bpp')
ylabel('SSIM')
title('SSIM(bpp)')
legend([p2], 'Location', 'northwest')
%%
% fileID = fopen('bpp6_d.txt','w');
% fprintf(fileID,'%.16f\n',bpp6);
% fclose(fileID);
% fileID = fopen('PSNR6_d.txt','w');
% fprintf(fileID,'%.16f\n',PSNR6);
% fclose(fileID);
% fileID = fopen('SSIM6_d.txt','w');
% fprintf(fileID,'%.16f\n',SSIM6);
% fclose(fileID);
%%
