...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
quality = 90;  % Параметр качества выбираем целым из диапазона ~ 1..100  
I = im2double(imread('lena_gray_512.tif'));  %обрабатываемое изображение
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

T = dctmtx(8); % К ПУНКТУ №6 ЗАДАНИЯ: Заменить преобразование
dct = @(x)T * x * T';

scale = 0.125*round(156*1.03125^quality); % нормировка - экспоненц. шкала
...
PSNR = 10*log10(512*512/sum(sum((I-I2).^2)))
s = dir('jp.ar');
the_size = s.bytes;
