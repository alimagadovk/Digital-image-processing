...
PSNR(my_ind) = 10*log10(512*512/sum(sum((I-I2).^2)))
s = dir('jp.ar');
the_size = s.bytes;
bpp(my_ind) = 8 * the_size / 512^2
end
%%
close all
figure
hold on
grid on
plot(bpp, PSNR, '*', 'LineWidth', 1.5)
p1 = plot(bpp, PSNR, 'b', 'LineWidth', 1.5, 'DisplayName', 'Прямой порядок');
xlabel('bpp')
ylabel('PSNR')
title('PSNR(bpp)')
%%
% fileID = fopen('bpp1.txt','w');
% fprintf(fileID,'%.16f\n',bpp);
% fclose(fileID);
% fileID = fopen('PSNR1.txt','w');
% fprintf(fileID,'%.16f\n',PSNR);
% fclose(fileID);
%%
