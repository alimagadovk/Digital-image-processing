...
%imwrite(im2uint8(z),strcat('7Q_Level=', num2str(Levels(my_ind)), '_bpp=', num2str(bpp7(my_ind)), '.gif'),'gif');
end
%%
close all
figure
...
legend([p1], 'Location', 'northwest')
%%
% fileID = fopen('bpp7.txt','w');
% fprintf(fileID,'%.16f\n',bpp7);
% fclose(fileID);
% fileID = fopen('PSNR7.txt','w');
% fprintf(fileID,'%.16f\n',PSNR7);
% fclose(fileID);
% fileID = fopen('SSIM7.txt','w');
% fprintf(fileID,'%.16f\n',SSIM7);
% fclose(fileID);
%%
