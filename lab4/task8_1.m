...
clc
clear
quality = [10 30 50 100];  % Параметр качества выбираем целым из диапазона ~ 1..100
...
PSNR8_1(my_ind) = 10*log10(512*512/sum(sum((I-I2).^2)))
SSIM8_1(my_ind) = ssim(im2uint8(I),im2uint8(I2));
s = dir('jp.ar');
the_size = s.bytes;
bpp8_1(my_ind) = 8 * the_size / 512^2
imwrite(im2uint8(I2),strcat('81Q_quality=', num2str(quality(my_ind)), '_bpp=', num2str(bpp8_1(my_ind)), '.gif'),'gif');
...
% hold on
% grid on
plot(bpp8_1, PSNR8_1, '*r', 'LineWidth', 1.5)
p2 = plot(bpp8_1, PSNR8_1, 'r', 'LineWidth', 1.5, 'DisplayName', 'Обратный порядок');
xlabel('bpp')
ylabel('PSNR')
title('PSNR(bpp)')
legend([p1, p2], 'Location', 'northwest')
%%
% fileID = fopen('bpp8_1.txt','w');
% fprintf(fileID,'%.16f\n',bpp8_1);
% fclose(fileID);
% fileID = fopen('PSNR8_1.txt','w');
% fprintf(fileID,'%.16f\n',PSNR8_1);
% fclose(fileID);
% fileID = fopen('SSIM8_1.txt','w');
% fprintf(fileID,'%.16f\n',SSIM8_1);
% fclose(fileID);
%%
