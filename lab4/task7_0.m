...
qstep = [0.25 0.3 0.35 0.4]; % шаг квантования ДВП-коэффициентов (регулировка ошибки)
Levels = 1; % Количество уровней ДВП-разложения
...
legend([p1], 'Location', 'northwest')
%%
% fileID = fopen('bpp70.txt','w');
% fprintf(fileID,'%.16f\n',bpp7);
% fclose(fileID);
% fileID = fopen('PSNR70.txt','w');
% fprintf(fileID,'%.16f\n',PSNR7);
% fclose(fileID);
% fileID = fopen('SSIM70.txt','w');
% fprintf(fileID,'%.16f\n',SSIM7);
% fclose(fileID);
%%
